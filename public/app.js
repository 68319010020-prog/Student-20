...
document.querySelector('#authForm').addEventListener('submit', async (event) => {
  ...
  try { const user = await request(`/api/${isRegistering ? 'register' : 'login'}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ username: document.querySelector('#authUsername').value, password: document.querySelector('#authPassword').value }) }); showWorkspace(user); } catch (error) { showAuthError(error); } finally { button.disabled = false; }
});
document.querySelector('#registerButton').addEventListener('click', (event) => { const authForm = document.querySelector('#authForm'); const registering = authForm.dataset.register === 'true'; authForm.dataset.register = String(!registering); authForm.querySelector('button[type="submit"]').innerHTML = registering ? '<span>→</span>เข้าสู่ระบบ' : '<span>+</span>สร้างบัญชี'; event.currentTarget.textContent = registering ? 'ยังไม่มีบัญชี? สร้างบัญชีใหม่' : 'มีบัญชีแล้ว? เข้าสู่ระบบ'; authForm.reset(); document.querySelector('#authError').textContent = ''; });
